...
class SettingsWindow(QWidget):
    ...
    def create_basic_tab(self):
        ...
        if self.data.get("Language", "zh-CN") == "en-US":
            self.language_combo.setCurrentIndex(0)
        elif self.data.get("Language", "zh-CN") == "ja-JP":
    ...
    def create_appearance_tab(self):
        ...
        self.notify_title_select.clicked.connect(
            lambda: self.select_file(self.notify_title_font)
        )
        ...
        self.notify_message_select.clicked.connect(
            lambda: self.select_file(self.notify_message_font)
        )
        ...
        self.override.setChecked(self.data.get("Override_qss", False))
        self.override_layout.addWidget(self.override)
        self.override_qss = QLineEdit(self.data.get("Override_Path", ""))
        self.override_select = QPushButton(self.tr("浏览"))
        self.override_select.clicked.connect(
            lambda: self.select_file(self.override_qss)
        )
    ...
    def create_about_tab(self):
        ...
        self.help.clicked.connect(
            lambda: webbrowser.open("https://xxtsoft.top/support/qqlistener")
        )
        self.translation = QPushButton(self.tr("提交翻译"))
        self.translation.clicked.connect(
            lambda: webbrowser.open(
                "https://xxtsoft.top/support/qqlistener/translation"
            )
        )
...
if __name__ == "__main__":
    ...
    if fuckqt.get("Language", "zh-CN") != "zh-CN":
    ...
    win.show()
    if win.data.get("Green_Hand", False):
        QMessageBox.information(
            win,
            win.tr("你是新来的吧？"),
            win.tr(
